...
'''
На предыдущем занятии мы говорили про роли и доступы, обсудили модель RBAC и внедрили ее в наш проект.
Сегодня мы продолжим развивать эти идеи. Когда мы ранее на курсе говорили про стандарт OAuth2 и JWT-токены, 
то затронули лишь небольшую часть данных инструментов. Сегодня нам понадобится новый механизм - OAuth2 scopes.
...
Мы создадим список для хранения scopes, и тогда Swagger сможет показывать список прав
в окне Authorize, а позже мы сможем требовать нужные scopes на эндпоинтах.

Чтобы модель прав была гибкой и можно было без особых проблем добавлять новые роли,
мы будем использовать трехкомпонентную схему scopes для нашего проекта:
...
- users:read:any (чтение любых пользователей)
- users:write:any (создание/изменение/удаление любых пользователей)

Мы объявили полный набор scopes заранее, чтобы Swagger документировал будущую модель доступа.
На этом уроке мы используем только часть scopes - users:read:own и users:wright:own подключим
дальше на курсе.
...
Если явно сопоставить наши роли и scopes, то на данном этапе это будет так:
user - items:read:own items:write:own users:read:own users:write:own
admin - items:read:any items:write:any users:read:any users:write:any
...
'''
Итак, сейчас мы в коде мы опишем, какие права существуют в нашем проекте, чтобы:
- FastAPI добавил их в OpenAPI-спецификацию
- Swagger начал показывать существующие права
- Мы могли указывать: "Этот эндпоинт требует вот такие scopes"
...
'''
Ранее мы объявили scopes в OpenAPI через OAuth2PasswordBearer, но сейчас это работа
на уровне документации. Следующий шаг связан уже с JWT-токеном. Сейчас в наших токенах
есть поле 'sub', через него мы понимаем какой именно пользователь просит доступ к API.
Но внутри JWT-токена можно также объявить поле 'scope', где будет лежать строка со списком scopes,
которые разделены пробелами. То есть поле 'scope' может выглядеть примерно так:

"scope": "items:read:own items:write:own users:read:own users:write:own" - для обычного пользователя
"scope": "items:read:any items:write:any users:read:any users:write:any" - для админа

Важно - клиент не должен выбирать scopes самостоятельно.
Если дать клиенту выбирать scopes при аутентификации, то если это злоумышленник, тогда он просто укажет
users:write:any и сможет вносить изменения в данные пользователей. Поэтому мы выдаем scopes строго по роли в БД,
это и есть взаимодействие RBAC и scopes (RBAC → scopes → JWT).
